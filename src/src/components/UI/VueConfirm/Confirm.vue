      ...
      <div class="vue-confirm fixed inset-0 z-[100] overflow-y-auto">
        <div class="min-h-screen px-4 text-center">
          ...
          <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95" enter-to="opacity-100 scale-100" leave="duration-200 ease-in" leave-from="opacity-100 scale-100" leave-to="opacity-0 scale-95">
            <div class="inline-block w-full max-w-md my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-lg rounded-md">

              <div class="hidden sm:block absolute top-0 right-0 pt-4 pr-4">
                <button @click="close" type="button" class="bg-white rounded-md text-zinc-400 hover:text-zinc-500 focus:outline-none">
                  <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                </button>
              </div>
              ...
              <div class="flex items-center justify-between bg-zinc-50 px-4 py-3">
                <div class="flex-1 flex items-center justify-end space-x-3">
                  <button v-if="buttons.no" type="button" class="tw-button sm gray transparent" @click="answer(false)">
                    {{ buttons.no }}
                  </button>
                  <button v-if="buttons.yes" type="button" class="tw-button sm" @click="answer(true)">
                    {{ buttons.yes }}
                  </button>
                  <button v-if="buttons.delete" type="button" class="tw-button red sm" @click="answer(true)">
                    {{ buttons.delete }}
                  </button>
                </div>
              </div>
