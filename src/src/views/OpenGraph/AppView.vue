<template>
  <DroppableSection @filedrop="handleFileDrop" :droppable="true" :opaque="isDroppable" class="AppView flex flex-col justify-between h-full">
    <!-- toolbar -->
    <WorkspaceBar/>

    <!-- no workflow, no mapping -->
    <div v-if="!nodegraph.currentWorkflow && !mapping.mainpane" class="flex flex-col items-center justify-center h-full">
      ...
      <span class="max-w-xs text-lg text-center mt-3">Please drag'n drop a SDFX app or load one from the left menu.</span>
    </div>

    <!-- has workflow but no mainpane mapping -->
    <div v-if="nodegraph.currentWorkflow && !hasMainpaneMapping" class="flex flex-col items-center justify-center h-full">
      ...
      <span class="max-w-xs text-lg text-center mt-3">
        This workflow has no UI. You can learn how to create apps and beautiful User Interfaces on our <a href="https://discord.gg/C3GVetwxnU" target="_blank" class="tw-underlink">discord server</a>.
      </span>
      <RouterLink to="/graph" class="tw-button lg mt-6">
        Edit Workflow
      </RouterLink>
    </div>
    ...
    <HTMLComponent
      v-if="nodegraph.currentWorkflow && mapping.mainpane && hasMainpaneMapping"
