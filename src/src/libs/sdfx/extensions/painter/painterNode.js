/*
 * SDFX port of PainterNode
 * Original Author: AlekPet
 * Version: 2023.10.19
...
function toRGBA(hex, alpha = 1.0) {
  ...
  if (array_hex) {
    return `rgba(${array_hex.map((h) => parseInt(h, 16)).join(', ')}, ${alpha})`
  }
  return hex
}
...
function LS_Save() {
  if (painters_settings_json) {
    ...
    localStorage.setItem('sdfx-painter', JSON.stringify(LS_Painters))
  }
}
// ================= END FUNCTIONS ================
...
class Painter {
  ...
  makeElements() {
    ...
    panelPaintBox.innerHTML = `<div class="painter_manipulation_box" f_name="Locks" style="display:none;">
        ...
        <div class="sdfx-menu-btns">
            ...
            <button id="zpos_BringFrontSelected" title="Moves an object or the objects of a multiple selection to the top after mouse click" class="${this.bringFrontSelected ? 'active' : ''}">Bring Up Always</button>
        </div>
    </div>
    <div class="painter_drawning_box_property" style="display: block;">
        <div class="property_textBox sdfx-menu-btns" style="display:none;">
    ...
    <div class="painter_drawning_box">
        <div class="painter_mode_box fieldset_box sdfx-menu-btns" f_name="Mode">
        ...
        <div class="painter_drawning_elements" style="display:block;">
            <div class="painter_grid_style painter_shapes_box fieldset_box sdfx-menu-btns" f_name="Shapes">
            ...
            <div class="painter_grid_style painter_bg_setting fieldset_box sdfx-menu-btns" f_name="Background">
    ...
    <div class="painter_history_panel sdfx-menu-btns">
    ...
    // Manipulation box
    this.manipulation_box = panelPaintBox.querySelector('.painter_manipulation_box')
    this.painter_drawning_box_property = panelPaintBox.querySelector('.painter_drawning_box_property')
    this.property_textBox = this.painter_drawning_box_property.querySelector('.property_textBox')

    ;[this.undo_button, this.redo_button] = panelPaintBox.querySelectorAll('.painter_history_panel button')

    // Modify in change mode
    this.painter_shapes_box_modify = panelPaintBox.querySelector('.painter_shapes_box_modify')
    this.painter_drawning_elements = panelPaintBox.querySelector('.painter_drawning_elements')
    ;[this.painter_shapes_box, this.painter_colors_box, this.painter_stroke_box, this.painter_bg_setting] = this.painter_drawning_elements.children

    // Make select font
    this.font_family_select = this.property_textBox.querySelector('.font_family_select')
    ...
    this.painter_shapes_box = panelPaintBox.querySelector('.painter_shapes_box')
    ...
    this.list_objects_panel__items = panelPaintBox.querySelector('.list_objects_panel__items')

    this.strokeColorTransparent = panelPaintBox.querySelector('#strokeColorTransparent')
    this.fillColorTransparent = panelPaintBox.querySelector('#fillColorTransparent')
    this.bgColor = panelPaintBox.querySelector('#bgColor')
    this.clear = panelPaintBox.querySelector('#clear')

    this.painter_bg_setting = panelPaintBox.querySelector('.painter_bg_setting')
  ...
  clearLocks() {
    try {
      const locksElements = this.manipulation_box.querySelectorAll('[id^=lock]')
      if (locksElements) {
  ...
  changeMode(b) {
    ...
    if (this.drawning) {
      ...
      if (!['Brush', 'Erase', 'Image', 'Textbox'].includes(this.type)) this.canvas.isDrawingMode = false
    }
    ...
    if (!this.mode) {
      ...
      this.viewListObjects(this.list_objects_panel__items)
      showHide({
        elements: [this.manipulation_box, nextElement, panelListObjects, this.painter_drawning_box_property],
      })
      this.clearLocks()
      ...
      this.painter_shapes_box.insertAdjacentElement('afterend', this.painter_colors_box)
      this.painter_colors_box.insertAdjacentElement('afterend', this.painter_stroke_box)
    }
  ...
  changePropertyBrush(type = 'Brush') {
    if (type == 'Brush') {
      ...
      this.canvas.freeDrawingBrush.color = toRGBA(this.strokeColor.value, this.strokeColorTransparent.value)
    }
    if (type != 'Erase' || (type == 'Erase' && !this.drawning)) {
      let a_obs = this.canvas.getActiveObjects()
      if (a_obs) {
        a_obs.forEach((a_o) => {
          this.setActiveStyle('strokeWidth', parseInt(this.strokeWidth.value, 10), a_o)
          this.setActiveStyle('stroke', toRGBA(this.strokeColor.value, this.strokeColorTransparent.value), a_o)
          this.setActiveStyle('fill', toRGBA(this.fillColor.value, this.fillColorTransparent.value), a_o)
        })
      }
    } else {
  ...
  // Make shape
  shapeCreate({ type, left, top, stroke, fill, strokeWidth, points = [], path = '' }) {
    let shape = null
  ...
  bindEvents() {
        ...
        switch (currentTarget) {
          ...
          case 'Textbox':
            let textbox = new fabric.Textbox('Text here', {
              fontFamily: 'Arial',
              stroke: toRGBA(this.strokeColor.value, this.strokeColorTransparent.value),
    ...
    this.manipulation_box.onclick = (e) => {
      let target = e.target,
        listButtons = [...Object.keys(this.locks), 'zpos_BringForward', 'zpos_BringToFront', 'zpos_SendToBack', 'zpos_SendBackwards', 'zpos_BringFrontSelected'],
        index = listButtons.indexOf(target.id)
      if (index != -1) {
        if (listButtons[index].includes('_Send') || listButtons[index].includes('_Bring')) {
    ...
    this.painter_drawning_box_property.onclick = (e) => {
      let target = e.target,
        listButtonsStyles = ['prop_fontStyle', 'prop_fontWeight', 'prop_underline'],
    ...
    this.font_family_select.onchange = (e) => {
      if (this.getActiveStyle('fontFamily') != this.font_family_select.value) this.setActiveStyle('fontFamily', this.font_family_select.value)
    }
    ...
    this.strokeColorTransparent.oninput =
      ...
      this.fillColorTransparent.oninput =
        () => {
          if (['Brush', 'Textbox', 'Image', 'Erase'].includes(this.type) || !this.drawning) {
    ...
    this.strokeWidth.onchange = () => {
      if (['Brush', 'Erase', 'Textbox', 'Image'].includes(this.type) || !this.drawning) {
        this.changePropertyBrush(this.type)
      }
    ...
    this.setInputsStyleObject = () => {
      ...
      const setProps = (style, check) => {
        this.painter_drawning_box_property.querySelector(`#prop_${style}`).classList[check ? 'remove' : 'add']('active')
      }

      targets.forEach((target) => {
        if (target.type == 'textbox') {
          setProps('fontWeight', this.getActiveStyle('fontWeight', target) == 'normal')
          setProps('fontStyle', this.getActiveStyle('fontStyle', target) == 'normal')
          setProps('underline', Boolean(this.getActiveStyle('underline', target)) == false)
        }

        if (!this.drawning && !['Erase', 'Brush'].includes(target.type)) {
          this.strokeWidth.value = parseInt(this.getActiveStyle('strokeWidth', target), 10)

          let { color: strokeColor, alpha: alpha_stroke } = getColorHEX(this.getActiveStyle('stroke', target)),
            { color: fillColor, alpha: alpha_fill } = getColorHEX(this.getActiveStyle('fill', target))
    ...
    this.canvas.on({
      ...
      'mouse:down': (o) => {
        if (!this.canvas.isDrawingMode && this.bringFrontSelected) this.canvas.bringToFront(this.canvas.getActiveObject())
        ...
        if (this.type != 'Textbox') {
          let { x: left, y: top } = this.canvas.getPointer(o.e),
            ...
            stroke = strokeWidth == 0 ? 'transparent' : toRGBA(this.strokeColor.value, this.strokeColorTransparent.value) || colors[Math.floor(Math.random() * colors.length)],
            fill = toRGBA(this.fillColor.value, this.fillColorTransparent.value),
      ...
      'mouse:move': (o) => {
        ...
        if (this.type == 'Circle') {
          let radius = Math.max(Math.abs(this.originY - pointer.y), Math.abs(this.originX - pointer.x)) / 2
  ...
  addToHistory() {
    ...
    if (this.undo_history.length > this.max_history_steps) {
      ...
      console.log(`[Info ${this.node.name}]: History saving step limit reached! Limit steps = ${this.max_history_steps}.`)
    }
  ...
  canvasSaveSettingsPainter() {
    try {
      ...
      if (LS_Painters && LS_Painters.hasOwnProperty(this.node.name)) {
        LS_Painters[this.node.name].canvas_settings = painters_settings_json ? data : JSON.stringify(data)
        LS_Save()
      }
    } catch (e) {
      console.error(e)
  ...
  canvasLoadSettingPainter() {
    try {
      if (LS_Painters && LS_Painters.hasOwnProperty(this.node.name) && LS_Painters[this.node.name].hasOwnProperty('canvas_settings')) {
        const data = painters_settings_json ? LS_Painters[this.node.name].canvas_settings : JSON.parse(LS_Painters[this.node.name].canvas_settings)
...
function PainterWidget(node, inputName, inputData, app) {
  ...
  const widget = {
    ...
    draw: function (ctx, _, widgetWidth, y, widgetHeight) {
      const margin = 10,
        left_offset = 8,
        top_offset = 30,
        visible = sdfx.canvas.ds.scale > 0.6 && this.type === 'painter_widget',
        w = widgetWidth - margin * 2 - 80,
        clientRectBound = ctx.canvas.getBoundingClientRect(),
        transform = new DOMMatrix()
          .scaleSelf(clientRectBound.width / ctx.canvas.width, clientRectBound.height / ctx.canvas.height)
      ...
      Object.assign(this.painter_wrap.style, {
        ...
        position: 'absolute',
        zIndex: sdfx.graph._nodes.indexOf(node),
      })
      ...
      Array.from(this.painter_wrap.children[2].querySelectorAll('input, button, input:after, span, div.painter_drawning_box')).forEach((element) => {
        if (element.type == 'number') {
          ...
          if (element.hasAttribute('painter_object')) sizesEl = { w: 58, h: 16, fs: 10 }
          if (element.hasAttribute('bgImage')) sizesEl = { w: 60, h: 20, fs: 10 }
  ...
  sdfx.canvas.canvasEl.parentElement.appendChild(widget.painter_wrap)
  ...
  sdfx.canvas.onDrawBackground = function () {
    // Draw node isnt fired once the node is off the screen
    // if it goes off screen quickly, the input may not be removed
    // this shifts it off screen so it can be moved back if the node is visible.
    for (let n in sdfx.graph._nodes) {
      n = sdfx.graph._nodes[n]
...
async function saveData() {
  try {
    ...
    if (rawResponse.status !== 200) throw new Error(`Error painter save settings: ${rawResponse.statusText}`)
  } catch (e) {
...
sdfx.registerExtension({
  name: 'SDFX.PainterNode',
  async init() {
  ...
  async setup() {
    console.log(sdfx.graph._nodes)

    if (sdfx.graph._nodes === null) return

    let PainerNode = sdfx.graph._nodes.filter((n) => n.type == 'PainterNode')
    ...
    if (PainerNode.length) {
      PainerNode.map((n) => {
        ...
        if (widgetImage && Object.hasOwn(LS_Painters, n.name)) {
          ...
          painter_ls.hasOwnProperty('objects_canvas') && delete painter_ls.objects_canvas // remove old property
          n.setSize([530, 570])
  ...
  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    if (nodeData.name === 'PainterNode') {
      ...
      if (painters_settings_json) {
        ...
        LS_Painters = localStorage.getItem('sdfx-painter') && JSON.parse(localStorage.getItem('sdfx-painter'))

        if (!LS_Painters || LS_Painters === undefined) {
          localStorage.setItem('sdfx-painter', JSON.stringify({}))
          LS_Painters = JSON.parse(localStorage.getItem('sdfx-painter'))
        }
      }
      ...
      nodeType.prototype.getExtraMenuOptions = function (_, options) {
        ...
        const past_index = options.findIndex((m) => m?.content === 'Paste (Clipspace)'),
