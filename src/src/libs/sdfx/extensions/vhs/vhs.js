...
function addCustomSize(nodeType, nodeData, widgetName) {
  ...
  for (let key in nodeData.input.required) {
    newWidgets[key] = nodeData.input.required[key]
    if (key == widgetName) {
      newWidgets[key][0] = newWidgets[key][0].concat([
        'Custom Width',
        'Custom Height',
        'Custom',
      ])
  ...
  chainCallback(nodeType.prototype, 'onNodeCreated', function () {
    ...
    var previewWidget = {
      ...
      draw: function (ctx, node, widgetWidth, widgetY, height) {
        ...
        const scale = sdfx.canvas.ds.scale //gets the litegraph zoom
    ...
    previewWidget.videoEl.addEventListener('loadedmetadata', () => {
      const w = previewWidget.videoEl.videoWidth
      const h = previewWidget.videoEl.videoHeight
    ...
    this._setPreviewsrc = function (params) {
      ...
      if (params?.format?.split('/')[0] == 'video') {
        ...
        //Is animated image
        previewWidget.imgEl.src = api.apiURL(
          '/view?' + new URLSearchParams(params),
        )
    ...
    this.onBounding = function () {
      //if (this._boundingCount++ > 5) {
        ...
        previewWidget.parentEl.style.width = '0px'
        previewWidget.parentEl.style.height = '0px'
      //}
    }
    previewWidget.parentEl.appendChild(previewWidget.videoEl)
    previewWidget.parentEl.appendChild(previewWidget.imgEl)
    if (sdfx.canvas) {
      sdfx.canvas.canvasEl.parentNode.appendChild(previewWidget.parentEl)
    }
  })
}
        ...
        Object.defineProperty(this, 'outputs', {
          set: function (value) {
            ...
            requestAnimationFrame(() => {
              if (sdfx.nodegraph.nodeOutputs[this.id + '']) {
                this.setPreviewsrc(sdfx.nodegraph.nodeOutputs[this.id + ''].gifs[0])
              }
            })
          },
          get: function () {
