...
import { applyTextReplacements } from '@/libs/sdfx/utils.js'

app.nodeOutputs = app.nodegraph.nodeOutputs
      ...
      if (this.inputs) {
        for (let i = 0; i < this.inputs.length; i++) {
          ...
          if (
            ...
            !(dt == 'IMAGE' && this.inputs[i].type == 'LATENT')
          ) {
            this.inputs[i].type = dt
...
function applyVHSAudioLinksFix(nodeType, nodeData, audio_slot) {
  chainCallback(
    nodeType.prototype,
    'onConnectionsChange',
    function (contype, slot, iscon, linfo) {
      if (contype == LiteGraph.OUTPUT && slot == audio_slot) {
        if (linfo.type == 'VHS_AUDIO') {
          ...
          let inputDef =
            LiteGraph.registered_node_types[tnode.type].nodeData?.input
...
function addVAEOutputToggle(nodeType, nodeData) {
  chainCallback(
    ...
    function (contype, slot, iscon, linfo) {
      if (
        contype == LiteGraph.INPUT &&
        slot == 1 &&
        this.inputs[1].type == 'VAE'
      ) {
        if (iscon && linfo) {
          if (this.linkTimeout) {
...
function addVAEInputToggle(nodeType, nodeData) {
  chainCallback(
    nodeType.prototype,
    'onConnectionsChange',
    function (contype, slot, iscon, linf) {
      if (
        contype == LiteGraph.INPUT &&
        slot == 3 &&
        this.inputs[3].type == 'VAE'
      ) {
        if (iscon && linf) {
          if (this.linkTimeout) {
            clearTimeout(this.linkTimeout)
  ...
  chainCallback(
    ...
    function (contype, slot, iscon, linf) {
      if (contype == LiteGraph.INPUT && slot == 0) {
        ...
        if (iscon && linf) {
          new_type = app.graph.getNodeById(linf.origin_id).outputs[
            linf.origin_slot
          ].type
        }
        if (this.linkTimeout) {
...
function addTimestampWidget(nodeType, nodeData, targetWidget) {
  ...
  chainCallback(nodeType.prototype, 'onNodeCreated', function () {
    const directoryWidget = this.widgets.find(
      (w) => w.name === 'directory_name',
    )
    const timestampWidget = this.widgets.find(
      (w) => w.name === 'timestamp_directory',
    )
    ...
    previewWidget.updateSource = function () {
      ...
      if (
        ...
        if (app.ui.settings.getSettingValue('VHS.AdvancedPreviews', false)) {
          this.videoEl.src = api.apiURL(
            '/viewvideo?' + new URLSearchParams(params),
          )
        } else {
  ...
  chainCallback(
    ...
    function (_, options) {
      ...
      if (previewWidget.videoEl?.hidden == false && previewWidget.videoEl.src) {
        ...
        url = api.apiURL(
          '/view?' + new URLSearchParams(previewWidget.value.params),
        )
        ...
        optNew.push(
          ...
          },
          {
            ...
            callback: () => {
              ...
              a.setAttribute(
                'download',
                new URLSearchParams(previewWidget.value.params).get('filename'),
              )
      ...
      const PauseDesc =
        (previewWidget.value.paused ? 'Resume' : 'Pause') + ' preview'
      ...
      const visDesc =
        (previewWidget.value.hidden ? 'Show' : 'Hide') + ' preview'
      optNew.push({
        content: visDesc,
        callback: () => {
      ...
      const muteDesc =
        (previewWidget.value.muted ? 'Unmute' : 'Mute') + ' Preview'
      optNew.push({
        content: muteDesc,
        callback: () => {
  ...
  chainCallback(nodeType.prototype, 'onNodeCreated', function () {
    ...
    Object.defineProperty(formatWidget, 'value', {
      set: (value) => {
        ...
        let newWidgets = []
        const fullDef = formatWidget.options.fullvalues.find((w) =>
          Array.isArray(w) ? w[0] === value : w === value,
        )
...
function addLoadVideoCommon(nodeType, nodeData) {
  ...
  chainCallback(nodeType.prototype, 'onNodeCreated', function () {
    ...
    const frameSkipWidget = this.widgets.find(
    ...
    chainCallback(
      this.widgets.find((w) => w.name === 'custom_width'),
      'callback',
      updateSize,
    )
    chainCallback(
      this.widgets.find((w) => w.name === 'custom_height'),
      'callback',
      updateSize,
    )
...
function searchBox(event, [x, y], node) {
  ...
  dialog.innerHTML =
    '<span class="name">Path</span> <input autofocus="" type="text" class="value"><button class="rounded">OK</button><div class="helper"></div>'
  dialog.close = () => {
    dialog.remove()
  }
      ...
      if (nodeData.description) {
        ...
        if (!el.children.length) {
          ...
          nodeData.description =
            el.querySelector('#VHS_shortdesc')?.innerHTML ||
            el.children[1]?.firstChild?.innerHTML
        }
        chainCallback(nodeType.prototype, 'onNodeCreated', function () {
        ...
        if (this.widgets) {
          for (let w of this.widgets) {
            ...
            if (w?.type == 'text' && config[1].vhs_path_extensions) {
              new_widgets.push(
                app.widgets.VHSPATH({}, w.name, ['VHSPATH', config[1]]),
              )
            } else {
              new_widgets.push(w)
            }
            ...
            requestAnimationFrame(() => {
              if (app.nodeOutputs[this.id + '']) {
                this.updateParameters(
                  app.nodeOutputs[this.id + ''].gifs[0],
                  true,
                )
              }
            })
        ...
        let w = {
          ...
          draw: function (ctx, node, widget_width, y, H) {
            ...
            if (show_text) {
              ...
              ctx.textAlign = 'right'
              let disp_text = this.format_path(
