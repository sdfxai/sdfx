...
import { ComfyWidgets, addValueControlWidgets } from '@/libs/sdfx/widgets.js'
import { applyTextReplacements } from '../../utils.js'

const CONVERTED_TYPE = 'converted-widget'
const VALID_TYPES = ['STRING', 'combo', 'number', 'slider', 'BOOLEAN']
const CONFIG = Symbol()
    ...
    nodeType.prototype.getExtraMenuOptions = function (_, options) {
      ...
      if (this.widgets) {
        ...
        */
        ...
        if (toInput.length || toWidget.length) {
          options.push(
            {
              content: 'Convert',
              has_submenu: true,
    ...
    class PrimitiveNode {
      ...
      applyToGraph(extraLinks = []) {
        ...
        for (const linkInfo of links) {
          ...
          if (widget) {
            widget.value = v
            if (app && app.canvas && widget.callback) {
              widget.callback(
                widget.value,
                app.canvas,
                node,
