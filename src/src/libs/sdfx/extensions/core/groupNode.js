...
const Workflow = {
  ...
  isInUseGroupNode(name) {
    ...
    // Check if already registered/in use in this workflow
...
class GroupNodeBuilder {
  ...
  getNodeData() {
    ...
    const backup = localStorage.getItem('graphclipboard')
    try {
      app.canvas.copyToClipboard(this.nodes)
      const config = JSON.parse(
        localStorage.getItem('graphclipboard'),
      )
      ...
      localStorage.setItem('graphclipboard', backup)
    }
  }
}
...
export class GroupNodeConfig {
  ...
  async registerType(source = 'workflow') {
    ...
    await app.registerNodeDef('workflow/' + this.name, this.nodeDef, true)
  }
  ...
  constructor(node) {
    ...
    this.node.convertToNodes = () => {
      const addInnerNodes = () => {
        ...
        localStorage.setItem('graphclipboard', JSON.stringify(c))
  ...
  updateInnerWidgets() {
    for (const newWidgetName in this.groupData.newToOldWidgetMap) {
      ...
      if (innerNode.type === 'PrimitiveNode') {
        ...
        if (rerouteLinks && rerouteLinks[0]) {
...
function addConvertToGroupOptions() {
  ...
  // Add to nodes
  const getNodeMenuOptions =CanvasContextMenu.getNodeMenuOptions
  CanvasContextMenu.getNodeMenuOptions = function (node) {
