...
import { LiteGraph, LCanvas, LNode } from '@/components/LiteGraph/'
...
function getClipPath(node, element, elRect) {
  const selectedNode = Object.values(sdfx.canvas.selected_nodes)[0]
  if (selectedNode && selectedNode !== node) {
    const MARGIN = 7
    const scale = sdfx.canvas.ds.scale
    ...
    const intersection = intersect(
      ...
      {
        x: selectedNode.pos[0] + sdfx.canvas.ds.offset[0] - MARGIN,
        y:
          selectedNode.pos[1] +
          sdfx.canvas.ds.offset[1] -
...
const computeVisibleNodes = LCanvas.prototype.computeVisibleNodes
LCanvas.prototype.computeVisibleNodes = function () {
  const visibleNodes = computeVisibleNodes.apply(this, arguments)
  for (const node of sdfx.graph._nodes) {
...
LNode.prototype.addDOMWidget = function (name, type, element, options) {
  options = {
    hideOnZoom: true,
  ...
  if (!element.parentElement) {
    const parent = sdfx.canvasEl.parentNode
  ...
  const widget = {
    ...
    draw: function (ctx, node, widgetWidth, y, widgetHeight) {
      ...
      const hidden =
        ...
        (!!options.hideOnZoom && sdfx.canvas.ds.scale < 0.5) ||
  ...
  for (const evt of options.selectOn) {
    element.addEventListener(evt, () => {
      sdfx.canvas.selectNode(this)
      sdfx.canvas.bringToFront(this)
    })
  }
